...
const getAcceptLanguageLocale = async (acceptLanguage, manifest, routesManifest) => {
    ...
    if (routesManifest.i18n) {
        ...
        const Accept = await Promise.resolve().then(function () { return require('./index-c93ef24d.js'); }).then(function (n) { return n.index; });
...
const isValidPreviewRequest = async (cookies, previewModeSigningKey) => {
    ...
    if (hasPreviewCookies(previewCookies)) {
        try {
            const jsonwebtoken = await Promise.resolve().then(function () { return require('./index-1ab7c2d9.js'); }).then(function (n) { return n.index; });
...
const triggerStaticRegeneration = async (options) => {
    ...
    const { SQSClient } = await Promise.resolve().then(function () { return require('./SQSClient-fe65310a.js'); });
    const { SendMessageCommand } = await Promise.resolve().then(function () { return require('./SendMessageCommand-627826e1.js'); });
...
const s3StorePage = async (options) => {
    const { S3Client } = await Promise.resolve().then(function () { return require('./S3Client-f2c371f4.js'); });
    ...
    const { PutObjectCommand } = await Promise.resolve().then(function () { return require('./PutObjectCommand-0dd908a8.js'); });
    await Promise.all([
...
const renderStaticPage = async ({ route, request, req, res, responsePromise, manifest, routesManifest, bucketName, s3Key, s3Uri, basePath }) => {
    ...
    const { GetObjectCommand } = await Promise.resolve().then(function () { return require('./GetObjectCommand-582d08ff.js'); });
    // S3 Body is stream per: https://github.com/aws/aws-sdk-js-v3/issues/1096
    const getStream = await Promise.resolve().then(function () { return require('./index-6a42f1bf.js'); }).then(function (n) { return n.index; });
    const s3Params = {
        Bucket: bucketName,
        Key: s3Key
    };
    ...
    try {
        ...
        bodyString = await getStream.default(s3Response.Body);
...
async function createExternalRewriteResponse(customRewrite, req, res, body) {
    const { default: fetch } = await Promise.resolve().then(function () { return require('./index-e7261f1e.js'); });
    // Set request headers
    const reqHeaders = {};
...
const handleOriginResponse = async ({ event, manifest, prerenderManifest, routesManifest }) => {
    ...
    if (fallbackRoute.isStatic) {
        ...
        const s3Response = await s3.send(new GetObjectCommand(s3Params));
        const bodyBuffer = await getStream.buffer(s3Response.Body);
        const statusCode = fallbackRoute.statusCode || 200;
        const is500 = statusCode === 500;
        const cacheControl = is500
            ? "public, max-age=0, s-maxage=0, must-revalidate" // static 500 page should never be cached
