...
require('./image-handler-e53f1482.js');
...
const serializeAws_restXmlGetObjectCommand = async (input, context) => {
    ...
    const headers = {
        ...
        ...(isSerializableHeaderValue(input.IfModifiedSince) && {
            "if-modified-since": index.dateToUtcString(input.IfModifiedSince).toString(),
        }),
        ...(isSerializableHeaderValue(input.IfNoneMatch) && { "if-none-match": input.IfNoneMatch }),
        ...(isSerializableHeaderValue(input.IfUnmodifiedSince) && {
            "if-unmodified-since": index.dateToUtcString(input.IfUnmodifiedSince).toString(),
        }),
    ...
    const query = {
        ...
        ...(input.ResponseExpires !== undefined && {
            "response-expires": index.dateToUtcString(input.ResponseExpires).toString(),
        }),
...
const deserializeAws_restXmlGetObjectCommand = async (output, context) => {
    ...
    if (output.headers["x-amz-delete-marker"] !== undefined) {
        contents.DeleteMarker = index.parseBoolean(output.headers["x-amz-delete-marker"]);
    }
    ...
    if (output.headers["last-modified"] !== undefined) {
        contents.LastModified = index.expectNonNull(index.parseRfc7231DateTime(output.headers["last-modified"]));
    }
    if (output.headers["content-length"] !== undefined) {
        contents.ContentLength = index.strictParseLong(output.headers["content-length"]);
    }
    if (output.headers["etag"] !== undefined) {
        contents.ETag = output.headers["etag"];
    }
    if (output.headers["x-amz-missing-meta"] !== undefined) {
        contents.MissingMeta = index.strictParseInt32(output.headers["x-amz-missing-meta"]);
    }
    ...
    if (output.headers["expires"] !== undefined) {
        contents.Expires = index.expectNonNull(index.parseRfc7231DateTime(output.headers["expires"]));
    }
    ...
    if (output.headers["x-amz-mp-parts-count"] !== undefined) {
        contents.PartsCount = index.strictParseInt32(output.headers["x-amz-mp-parts-count"]);
    }
    if (output.headers["x-amz-tagging-count"] !== undefined) {
        contents.TagCount = index.strictParseInt32(output.headers["x-amz-tagging-count"]);
    }
    ...
    if (output.headers["x-amz-object-lock-retain-until-date"] !== undefined) {
        contents.ObjectLockRetainUntilDate = index.expectNonNull(index.parseRfc3339DateTime(output.headers["x-amz-object-lock-retain-until-date"]));
    }
...
const deserializeAws_restXmlInvalidObjectStateResponse = async (parsedOutput, context) => {
    ...
    if (data["AccessTier"] !== undefined) {
        contents.AccessTier = index.expectString(data["AccessTier"]);
    }
    if (data["StorageClass"] !== undefined) {
        contents.StorageClass = index.expectString(data["StorageClass"]);
    }
    return contents;
};
